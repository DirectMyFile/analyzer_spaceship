...
class Scanner {
  ...
  int _tokenizeLessThan(int next) {
    ...
    next = _reader.advance();
    if (0x3D == next) {
      if (_reader.peek() == ">".codeUnitAt(0)) {
        _appendTokenOfType(TokenType.LT_EQ_GT);
        _reader.advance();
      } else {
...
class TokenType {
  ...
  static const TokenType LT_EQ_GT =
      const TokenType('LT_EQ_GT', TokenClass.RELATIONAL_OPERATOR, "<=>");
